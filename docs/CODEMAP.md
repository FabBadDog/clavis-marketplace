...
| `workspace-contracts` (`...Contracts.Workspace`) | Workspace identity, slots, activation, the workspace list, and per-workspace sessions |
| `keymap-contracts` (`...Contracts.Keymap`) | Keybindings + command catalog (`KeymapChanged`, `CommandsAvailable`, `RunCommand`, panel commands, plugin-declared defaults) |
...
|---|---|
| `WpfHost` | Owns the application windows + docking surface; persists/restores the layout. |
| `Conversation` | The elm/flux chat: state, pure update, view templates, permission/hook resolution. Owns the `chat` panel kind, prompt input included. |
...
| `Workspaces` | The single authority for workspace identity, slots, accents, activation and session creation; owns the F12 overview panel. |
