...
/// A boolean IsSelected -> underline visibility. Hidden (not Collapsed) when unselected so the 2px underline
/// keeps reserving its space and selecting an option never shifts the row.
[<ExcludeFromCodeCoverage>]
type private UnderlineVisibilityConverter() =
    interface IValueConverter with
        member _.Convert(value, _, _, _) =
            (if (value :?> bool) then Visibility.Visible else Visibility.Hidden) |> box
        member _.ConvertBack(_, _, _, _) = raise (NotSupportedException())

/// The view for a SegmentedSelectorModel: a wrapping row of option labels, each underlined in clavis while
/// selected. The options flow onto further lines when the host is too narrow, so no option is ever clipped.
...
type SegmentedSelector() as this =
    ...
    let buildOption (model: SegmentedSelectorModel) (index: int) (item: SegmentItem) =
        ...
        // Per-option underline, bound to the item's IsSelected so it tracks the highlight the model drives.
        let underline =
            Border(
                Height = 2.0,
        ...
        underline.SetBinding(
            UIElement.VisibilityProperty,
            Binding("IsSelected", Source = item, Converter = UnderlineVisibilityConverter())) |> ignore
    ...
    let build (model: SegmentedSelectorModel) =
        ...
        model.Items
        |> Seq.iteri (fun index item -> row.Children.Add(buildOption model index item) |> ignore)
        row :> obj

    do
        // Rebuild the option row whenever the model changes; the per-option underline bindings track the
        // highlight the model drives, so the view needs no selection subscription of its own.
        this.DataContextChanged.Add(fun args ->
            match args.NewValue with
            | :? SegmentedSelectorModel as model -> this.Content <- build model
            | _ -> this.Content <- null)
