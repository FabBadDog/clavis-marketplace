...
the configuration store, the agent bridge, and the panel registry - so the agent layer is live and the
restored panels materialise as the window appears, not seconds later). `essential` is a plugin concept;
declaring it on a module is a metadata
